...
describe("Habits functions", () => {
  describe("habits are from the same user as auth", () => {
  ...
  describe("habits are from another user", () => {
    let savedAnotherUser: any;
    beforeEach(async () => {
      ...
      savedAnotherUser = await anotherUser.save();
    });

    it("should not read a habit document", async () => {
      const habit = new Habit({ name: 'Habit 3', user: savedAnotherUser.id });
      const savedHabit = await habit.save();
      const response = await request(app).get(`/api/habits/${savedHabit._id}`);
      expect(response.status).toBe(404);
    });

    it("should not list habit documents", async () => {
      const habit1 = new Habit({ name: 'Habit 7', user: savedAnotherUser.id });
      const savedHabit1 = await habit1.save();
      const habit2 = new Habit({ name: 'Habit 8', user: savedAnotherUser.id });
      const savedHabit2 = await habit2.save();
      const response = await request(app).get(`/api/habits/`);
      expect(response.status).toBe(200);
    ...
    it("should not update the habit document", async () => {
      const habit = new Habit({ name: 'Habit 3', user: savedAnotherUser.id });
      const savedHabit = await habit.save();
      const response = await request(app)
        .put(`/api/habits/${savedHabit._id}`)
    ...
    it("should not delete a habit document", async () => {
      const habit = new Habit({ name: 'Habit to be removed', user: savedAnotherUser.id });
      const savedHabit = await habit.save();
      const response = await request(app)
        .delete(`/api/habits/${savedHabit._id}`)
      expect(response.status).toBe(404);
